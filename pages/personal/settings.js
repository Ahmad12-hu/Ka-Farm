...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.resetDatabase = () => {
    if (
      confirm(
        "Voulez-vous vraiment réinitialiser toutes les données ? Votre historique de cultures, tâches, arrosages et ventes sera perdu et remplacé par les données de démonstration de la famille KA."
      )
    ) {
      const keys = [
        "ka_farm_crops",
      ...
      if (window.ErrorHandler) {
        window.ErrorHandler.showToast(
          "Base de données réinitialisée avec succès ! L'application va redémarrer.",
          "success"
        );
      }
  ...
  window.loadDemoData = () => {
    if (
      confirm(
        "Voulez-vous charger le jeu de données de démonstration ? Cela remplacera vos données actuelles par des exemples réalistes."
      )
    ) {
      try {
  ...
  window.exportBackupData = () => {
    ...
    downloadAnchor.setAttribute(
      "download",
      `ka-farm-sauvegarde-${new Date().toISOString().split("T")[0]}.json`
    );
    ...
    if (window.ErrorHandler) {
      window.ErrorHandler.showToast(
        "Sauvegarde d'exploitation exportée avec succès ! Gardez ce fichier précieusement.",
        "success"
      );
    }
  };
    ...
    reader.onload = (e) => {
      try {
        ...
        if (!backup.crops || !backup.tasks || !backup.stocks) {
          if (window.ErrorHandler) {
            window.ErrorHandler.showToast(
              "Erreur : Le fichier sélectionné n'est pas un fichier de sauvegarde KA Farm valide.",
              "error"
            );
          }
          return;
        ...
        if (
          confirm(
            "Attention ! L'importation de cette sauvegarde va écraser toutes vos données actuelles. Voulez-vous continuer ?"
          )
        ) {
        ...
        if (window.ErrorHandler) {
          window.ErrorHandler.showToast(
            "Erreur lors de la lecture du fichier de sauvegarde : " + err.message,
            "error"
          );
        }
      }
  ...
  window.copyInvitationLink = () => {
    ...
    navigator.clipboard
      ...
      .then(() => {
        if (window.ErrorHandler) {
          window.ErrorHandler.showToast(
            "Lien d'inscription copié ! Envoyez ce lien à votre collaborateur pour qu'il crée son compte.",
            "success"
          );
        }
      })
