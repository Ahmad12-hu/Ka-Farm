...
export const CropsModule = {
  ...
  populateParcelOptions() {
    ...
    if (filterFieldSelect) {
      filterFieldSelect.innerHTML = '<option value="all">📍 Toutes Parcelles</option>' + 
        sortedParcels.map(p => `<option value="${p}">${p}</option>`).join('');
    }

    if (formFieldSelect) {
      formFieldSelect.innerHTML = sortedParcels.map(p => `<option value="${p}">${p}</option>`).join('') +
  ...
  setupListeners() {
    ...
    window.filterActiveCrops = () => {
      ...
      const filtered = crops.filter(crop => {
        const matchesQuery = crop.name.toLowerCase().includes(query) || 
                             (crop.field && crop.field.toLowerCase().includes(query));
        
        const matchesField = filterField === 'all' || crop.field === filterField;
        
        const matchesSeed = filterSeed === 'all' || (crop.seedType || 'Hybride F1 Certifiée') === filterSeed;
        
        const matchesSeason = filterSeason === 'all' || (crop.season || 'Saison Sèche Froide') === filterSeason;
    ...
    if (cropForm) {
      cropForm.addEventListener('submit', (e) => {
        ...
        const seedType = document.getElementById('form-crop-seed-type')?.value || 'Hybride F1 Certifiée';
