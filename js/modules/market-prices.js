...
export const MarketPricesModule = {
  ...
  updateStats() {
    ...
    const avgPrice =
      totalPrices > 0 ? prices.reduce((sum, p) => sum + (p.price_fcfa || 0), 0) / totalPrices : 0;
  ...
  renderPrices() {
    ...
    this.elements.pricesTableBody.innerHTML = prices
      .map(
        (price) => `
  ...
  renderTrends() {
    ...
    if (this.state.searchQuery) {
      ...
      trends = trends.filter(
        (t) => t.crop_name.toLowerCase().includes(query) || t.region.toLowerCase().includes(query)
      );
    }
    trends.sort((a, b) => new Date(b.last_updated) - new Date(a.last_updated));

    this.elements.trendsTableBody.innerHTML = trends
      .map(
        (trend) => `
      <tr class="border-b border-gray-700 hover:bg-gray-800/50">
  ...
  renderAlerts() {
    ...
    if (this.state.searchQuery) {
      ...
      alerts = alerts.filter(
        (a) =>
          a.crop_name.toLowerCase().includes(query) || a.market_name.toLowerCase().includes(query)
      );
    }
    alerts.sort((a, b) => {
    ...
    this.elements.alertsTableBody.innerHTML = alerts
      .map(
        (alert) => `
  ...
  renderSinglePriceChart() {
    ...
    if (data.length === 0) {
      container.innerHTML =
        '<p class="text-center text-slate-400 py-8">Aucune donnée disponible</p>';
      return;
    }
        ...
        plugins: {
          ...
          tooltip: {
            ...
            callbacks: {
              label(context) {
                return (
                  context.dataset.label + ": " + context.parsed.y.toLocaleString("fr-FR") + " FCFA"
                );
              },
            },
          },
  ...
  renderPriceTrendChart() {
    ...
    const dates = cropPrices.map((p) =>
      new Date(p.price_date).toLocaleDateString("fr-FR", { month: "short", day: "numeric" })
    );
    ...
    if (priceId) {
      ...
      if (price) {
        ...
        if (this.elements.seasonSelect)
          this.elements.seasonSelect.value = price.season || "Hivernage";
        if (this.elements.supplySelect)
          this.elements.supplySelect.value = price.supply_level || "Normale";
        if (this.elements.demandSelect)
          this.elements.demandSelect.value = price.demand_level || "Normale";
      ...
      if (this.elements.priceDateInput)
        this.elements.priceDateInput.value = new Date().toISOString().split("T")[0];
  ...
  openTrendModal(trendId = null) {
    ...
    if (trendId) {
      ...
      if (trend) {
        ...
        if (this.elements.trendAvgPriceInput)
          this.elements.trendAvgPriceInput.value = trend.avg_price;
        if (this.elements.trendMinPriceInput)
          this.elements.trendMinPriceInput.value = trend.min_price;
        if (this.elements.trendMaxPriceInput)
          this.elements.trendMaxPriceInput.value = trend.max_price;
        if (this.elements.trendDirectionSelect)
          this.elements.trendDirectionSelect.value = trend.trend_direction;
        if (this.elements.trendStrengthInput)
          this.elements.trendStrengthInput.value = trend.trend_strength;
        if (this.elements.trendPredictionInput)
          this.elements.trendPredictionInput.value = trend.prediction_next_month;
        if (this.elements.trendConfidenceInput)
          this.elements.trendConfidenceInput.value = trend.confidence_percent;
        if (this.elements.trendDataPointsInput)
          this.elements.trendDataPointsInput.value = trend.data_points;
  ...
  openAlertModal(alertId = null) {
    ...
    if (alertId) {
      ...
      if (alert) {
        ...
        if (this.elements.alertMarketSelect)
          this.elements.alertMarketSelect.value = alert.market_name;
        ...
        if (this.elements.alertThresholdInput)
          this.elements.alertThresholdInput.value = alert.threshold_price;
