...
export const WeatherAlertsModule = {
  ...
  renderStats() {
    ...
    const criticalAlerts = weatherAlertHistory.filter(
      (a) => !a.acknowledged && a.severity === "Critique"
    ).length;
    const highAlerts = weatherAlertHistory.filter(
      (a) => !a.acknowledged && a.severity === "Élevée"
    ).length;
  ...
  renderActiveAlerts() {
    ...
    tableBody.innerHTML = activeAlerts
      .map((alert) => {
  ...
  renderAlertHistory() {
    ...
    const sortedHistory = [...weatherAlertHistory].sort(
    ...
    tableBody.innerHTML = sortedHistory
      .map((alert) => {
        const severityInfo = SEVERITY_COLORS[alert.severity] || SEVERITY_COLORS["Moyenne"];
        const icon = ALERT_ICONS[alert.alert_type] || "alert-triangle";
  ...
  renderRecentTriggered() {
    ...
    const sorted = [...weatherAlertHistory].sort(
      (a, b) => new Date(b.triggered_at) - new Date(a.triggered_at)
    );
    ...
    if (recent5.length === 0) {
      recentList.innerHTML =
        '<p class="text-[11px] text-slate-400 text-center py-4">Aucune alerte récente</p>';
      return;
    }

    recentList.innerHTML = recent5
      .map((alert) => {
        const severityInfo = SEVERITY_COLORS[alert.severity] || SEVERITY_COLORS["Moyenne"];
        const icon = ALERT_ICONS[alert.alert_type] || "alert-triangle";
...
window.submitWeatherAlert = (e) => {
  ...
  if (!typeSelect || !messageInput || !triggerValueInput || !severitySelect) {
    ErrorHandler.showToast(
      "Veuillez remplir les champs obligatoires : Type, Message, Valeur de déclenchement et Sévérité.",
      "error"
    );
    return;
  }
  ...
  ErrorHandler.showToast(
    `Alerte climatique "${alertType}" ${existingId ? "mise à jour" : "créée"} avec succès !`,
    "success"
  );
};
...
window.editWeatherAlert = (id) => {
  ...
  document.getElementById("form-trigger-value").value =
    alert.trigger_threshold || alert.trigger_value || 0;
