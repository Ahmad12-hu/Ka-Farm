...
export const ParcellesModule = {
  ...
  renderMap() {
    ...
    parcelles.forEach((parcel, index) => {
      ...
      textArea.setAttribute('x', pos.x + 12);
      textArea.setAttribute('y', pos.y + 39);
      const areaFill = isDark ? '#94A3B8' : '#475569';
      textArea.setAttribute('fill', areaFill);
      textArea.setAttribute('font-family', 'ui-sans-serif, system-ui, sans-serif');
      textArea.setAttribute('font-size', '8');
      textArea.setAttribute('font-weight', '700');
      textArea.textContent = `${parcel.surface} m²`;
      parcelG.appendChild(textArea);
      ...
      if (isSelected) {
        ...
        pinGG.setAttribute('transform', `translate(${pos.x + pos.w - 22}, ${pos.y + 8})`);
        
        const pinBg = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
        pinBg.setAttribute('cx', '8');
        pinBg.setAttribute('cy', '8');
        pinBg.setAttribute('r', '7');
        pinBg.setAttribute('fill', '#10B981');
        pinBg.setAttribute('opacity', '0.9');
        pinGG.appendChild(pinBg);

        const pinDot = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
        pinDot.setAttribute('cx', '8');
        pinDot.setAttribute('cy', '8');
        pinDot.setAttribute('r', '3');
        pinDot.setAttribute('fill', '#FFFFFF');
        pinGG.appendChild(pinDot);

        parcelG.appendChild(pinGG);
      }
