...
export const ParcellesModule = {
  init() {
    ...
    if (idParam && parcelles.some((p) => p.id === idParam)) {
      ...
      const found = parcelles.find(
        (p) =>
          p.name.toLowerCase().includes(searchParam.toLowerCase()) ||
          (p.currentCrop && p.currentCrop.toLowerCase().includes(searchParam.toLowerCase()))
      );
      if (found) {
  ...
  renderTable() {
    ...
    parcelles.forEach((p) => {
      ...
      if (p.status === "Cultivée") {
        statusBadge =
          '<span class="px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider bg-emerald-500/10 text-emerald-500 border border-emerald-500/20">Cultivée</span>';
      } else if (p.status === "En préparation") {
        statusBadge =
          '<span class="px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider bg-amber-500/10 text-amber-500 border border-amber-500/20">En prép</span>';
      } else {
        statusBadge =
          '<span class="px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider bg-slate-500/10 text-slate-400 border border-slate-500/20">Jachère</span>';
      }
  ...
  renderDetails() {
    ...
    if (badgeEl) {
      ...
      if (parcel.status === "Cultivée") {
        ...
        badgeEl.className =
          "px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider bg-amber-500/10 text-amber-500 border border-amber-500/20";
      } else {
        badgeEl.className =
    ...
    if (historyContainer) {
      ...
      if (!parcel.history || parcel.history.length === 0) {
        historyContainer.innerHTML =
          '<p class="text-xs text-slate-400 dark:text-slate-500 font-semibold italic pl-1">Aucun historique enregistré pour le moment.</p>';
      } else {
  ...
  renderBreakEvenAnalysis(parcel) {
    ...
    const standards = CROP_MARKET_STANDARDS[currentCrop] || {
      marketPrice: 500,
      yieldPerSqM: 3.0,
      inputCost: 25000,
    };
    ...
    if (progressBar) {
      ...
      if (percentage <= 0) {
        ...
        progressBar.className =
          "bg-gradient-to-r from-emerald-500 to-teal-400 h-full rounded-full transition-all duration-500";
      } else if (percentage <= 60) {
        progressBar.className =
          "bg-gradient-to-r from-emerald-400 to-amber-400 h-full rounded-full transition-all duration-500";
      } else if (percentage <= 90) {
        progressBar.className =
          "bg-gradient-to-r from-amber-400 to-orange-400 h-full rounded-full transition-all duration-500";
      } else {
        progressBar.className =
          "bg-gradient-to-r from-orange-400 to-rose-500 h-full rounded-full transition-all duration-500";
      }
    }
  ...
  submitEditParcel() {
    try {
      ...
      if (idx !== -1) {
        ...
        if (
          currentCrop &&
          currentCrop !== parcelles[idx].currentCrop &&
          !history.includes(currentCrop)
        ) {
          history.unshift(currentCrop);
        }
  ...
  switchParcelView(view) {
    ...
    if (view === "map" && !this.canViewMap()) {
      ErrorHandler.showToast(
        "Accès refusé. Seuls les utilisateurs Bureau et Admin peuvent consulter la carte.",
        "error"
      );
      return;
    }
    ...
    if (view === "list") {
      ...
      if (btnList) {
        btnList.className =
          "px-3 py-1.5 bg-emerald-600 text-white text-[10px] font-black uppercase tracking-wider rounded-lg";
      }
      if (btnMap) {
        btnMap.className =
          "px-3 py-1.5 bg-slate-100 dark:bg-[#0B2112] text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-[#143E23]/40 text-[10px] font-black uppercase tracking-wider rounded-lg";
      }
    } else if (view === "map") {
...
window.openAddParcelModal = () => {
  ...
  if (modal) {
    ...
    const baseLat = 14.793;
    const baseLng = -17.265;
...
window.deleteParcel = (id) => {
  if (
    confirm(
      `Êtes-vous sûr de vouloir supprimer définitivement la parcelle ${id} ? Cette action effacera également son historique.`
    )
  ) {
    try {
...
window.getCurrentPositionGPS = () => {
  ...
  navigator.geolocation.getCurrentPosition(
    (position) => {
      ...
      if (selectedParcelId) {
        ...
        if (idx !== -1) {
          ...
          ParcellesModule.render();
          ErrorHandler.showToast(
            `Coordonnées GPS mises à jour pour ${parcelles[idx].name}`,
            "success"
          );
        }
      }
    ...
    (error) => {
      ErrorHandler.showToast(
        "Impossible de récupérer votre position. Vérifiez les autorisations GPS.",
        "error"
      );
