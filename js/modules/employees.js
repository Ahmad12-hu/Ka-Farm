...
export const EmployeesModule = {
  ...
  renderStats() {
    ...
    const presentCount = dayAttendance.filter(
      (a) => a.status === "Présent" || a.status === "Demi-journée"
    ).length;
  ...
  renderDetails() {
    ...
    if (statusBadge) {
      ...
      if (emp.status === "Actif") {
        statusBadge.className =
          "px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider bg-emerald-500/10 text-emerald-500 border border-emerald-500/20";
      } else {
        statusBadge.className =
          "px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-wider bg-slate-500/10 text-slate-400 border border-slate-500/20";
      }
    }
    ...
    if (tasksContainer) {
      ...
      const empTasks = tasks.filter(
        (t) => t.assignee.toLowerCase().trim() === emp.name.toLowerCase().trim()
      );

      if (empTasks.length === 0) {
        tasksContainer.innerHTML =
          '<p class="text-xs text-slate-400 dark:text-slate-500 font-semibold italic py-2">Aucune tâche assignée actuellement.</p>';
      } else {
        empTasks.forEach((t) => {
          ...
          if (t.priority === "Haute") {
            priorityBadge =
              '<span class="text-[8px] px-1.5 py-0.1 bg-rose-500/10 text-rose-500 rounded font-black uppercase">HAUTE</span>';
          } else if (t.priority === "Moyenne") {
            priorityBadge =
              '<span class="text-[8px] px-1.5 py-0.1 bg-amber-500/10 text-amber-500 rounded font-black uppercase">MOY</span>';
          } else {
            priorityBadge =
              '<span class="text-[8px] px-1.5 py-0.1 bg-slate-500/10 text-slate-400 rounded font-black uppercase">BASSE</span>';
          }
  ...
  renderAttendanceHistoryDays() {
    ...
    if (dates.length === 0) {
      container.innerHTML =
        '<p class="text-xs text-[#819888] font-bold italic col-span-full">Aucun historique enregistré.</p>';
      return;
    }
    ...
    dates.forEach((d) => {
      ...
      div.className =
        "p-3 bg-[#0D2615]/30 border border-[#143E23]/20 hover:border-emerald-500/30 hover:bg-[#0D2615]/50 rounded-2xl text-left space-y-1 transition-all cursor-pointer";
  ...
  renderPaymentsHistory() {
    ...
    if (sortedPayments.length === 0) {
      container.innerHTML =
        '<p class="text-xs text-slate-400 font-semibold italic text-center py-6">Aucun versement enregistré pour le moment.</p>';
      return;
    }
    ...
    sortedPayments.forEach((pay) => {
      ...
      div.className =
        "p-3.5 bg-slate-50 dark:bg-[#0D2615]/30 border border-slate-100 dark:border-[#143E23]/25 rounded-2xl text-xs font-semibold space-y-2";
  ...
  loadEmployeeSelects() {
    ...
    if (calcSelect) {
      calcSelect.innerHTML = employees
  ...
  submitAttendancePointage() {
    try {
      ...
      const currentAttendance = KAStorage.getAttendance();

      activeEmployees.forEach((emp) => {
        const selectedStatus =
          document.querySelector(`input[name="status-${emp.id}"]:checked`)?.value || "Présent";
        const notes = document.querySelector(`input[name="notes-${emp.id}"]`)?.value.trim() || "";

        // Find and replace or add
        const idx = currentAttendance.findIndex(
          (a) => a.employeeId === emp.id && a.date === selectedDate
        );
        if (idx !== -1) {
          currentAttendance[idx] = {
            employeeId: emp.id,
            date: selectedDate,
...
window.switchTab = (tab) => {
  ...
  tabs.forEach((t) => {
    ...
    if (btn && sec) {
      if (t === tab) {
        btn.className =
          "px-6 py-3 font-black text-xs uppercase tracking-wider border-b-2 border-emerald-500 text-emerald-500 outline-none transition-all cursor-pointer";
        sec.classList.remove("hidden");
      } else {
        btn.className =
          "px-6 py-3 font-black text-xs uppercase tracking-wider border-b-2 border-transparent text-slate-400 hover:text-slate-200 outline-none transition-all cursor-pointer";
...
window.deleteEmployee = (id) => {
  if (
    confirm(
      `Êtes-vous sûr de vouloir licencier/supprimer définitivement l'ouvrier ${id} ? Ses pointages et paiements seront conservés.`
    )
  ) {
    try {
...
window.calculateSalary = () => {
  ...
  const logs = attendance.filter(
    (a) => a.employeeId === empId && a.date >= startDate && a.date <= endDate
  );
  ...
  const emppayments = payments.filter(
    (p) => p.employeeId === empId && p.date >= startDate && p.date <= endDate
  );
  ...
  document.getElementById("calc-already-paid").textContent =
    totalPaidOnPeriod.toLocaleString("fr-FR");
