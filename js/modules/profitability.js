...
export const ProfitabilityModule = {
  ...
  renderStats() {
    ...
    if (elBestCrop)
      elBestCrop.textContent = bestCrop
        ? `${CROP_CATEGORIES[bestCrop.cropName]?.emoji || "🌱"} ${bestCrop.cropName}`
        : "Aucune";
    if (elWorstCrop) {
      elWorstCrop.textContent = worstCrop
        ? `${CROP_CATEGORIES[worstCrop.cropName]?.emoji || "🌱"} ${worstCrop.cropName}`
        : "Aucune";
  ...
  renderTable() {
    ...
    tableBody.innerHTML = sortedProfits
      .map((profit, index) => {
  ...
  renderTopCrops() {
    ...
    if (top5.length === 0) {
      topCropsList.innerHTML =
        '<p class="text-[11px] text-slate-400 text-center py-4">Aucune donnée disponible</p>';
      return;
    }

    topCropsList.innerHTML = top5
      .map((profit, index) => {
        const cropInfo = CROP_CATEGORIES[profit.cropName] || { emoji: "🌱" };
        const percentage =
          profit.netMargin > 0
            ? ((profit.netMargin / (profit.totalCost || 1)) * 100).toFixed(1)
            : 0;
  ...
  initChart() {
    ...
    const backgroundColors = cropProfits.map(
...
window.submitAddProfit = (e) => {
  ...
  if (!cropSelect || !yieldInput || !priceInput) {
    ErrorHandler.showToast(
      "Veuillez remplir les champs obligatoires: Culture, Production et Prix de vente.",
      "error"
    );
    return;
  }
  ...
  ErrorHandler.showToast(
    `Analyse de rentabilité pour ${cropName} enregistrée avec succès ! Marge nette: ${Math.round(netMargin).toLocaleString("fr-FR")} F`,
    "success"
  );
};
...
window.deleteProfit = (id) => {
  ...
  if (deleteModal && confirmBtn) {
    ...
    confirmBtn.onclick = () => {
      ...
      ErrorHandler.showToast(
        `Analyse de rentabilité pour ${cropName} supprimée avec succès.`,
        "success"
      );
    };
