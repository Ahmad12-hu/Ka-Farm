...
export const TrainingModule = {
      ...
      try {
        if (selectedIndex === correctIndex) {
          ...
          ErrorHandler.showToast(
            "✗ Incorrect. Révisez les fiches techniques ci-dessus pour progresser.",
            "error"
          );
        }
      } catch (err) {
  ...
  applyFrenchLabels() {
    const labels = {
      ...
      "training-header-desc":
        "Itinéraires techniques détaillés, calendriers culturaux et diagnostics biologiques pour réussir vos cultures au Sénégal.",
      "crop-fiches-title": "Fiches Techniques & Itinéraires",
      "crop-fiches-desc":
        "Sélectionnez une culture pour charger son itinéraire technique sénégalais complet.",
      "principles-title": "Principes de l'Agriculture Biologique",
      "principles-desc":
        "Les 4 piliers essentiels pour protéger les sols et maximiser le rendement horticole.",
      ...
      "ai-mentor-desc":
        "Posez des questions spécifiques sur l'enseignement agricole, les semis, maladies, et techniques du Sénégal.",
  ...
  applyWolofLabels() {
    const labels = {
      ...
      "training-header-desc":
        "Bayti yooni bu jëm, kalandar yu xobbal ak diagnose yu biologie ngir réussir jang yi ci Sénégal.",
      ...
      crops.forEach((crop, index) => {
        ...
        btn.className = `crop-guide-tab px-4 py-2.5 text-xs font-black border-b-2 rounded-t-lg transition-all cursor-pointer ${
          index === 0
            ...
            : "border-transparent text-slate-450 dark:text-slate-400 hover:text-emerald-500"
        }`;
  ...
  loadQuiz() {
    try {
      ...
      const questions = [
        {
          question:
            "Quelle pratique réduit drastiquement l'évaporation de l'eau sous le soleil du Sénégal ?",
          options: [
            "L'arrosage intensif à midi",
            "Le paillage épais des planches",
            "Laisser le sol nu",
            "Pulvériser du sel",
          ],
          correct: 1,
        },
        {
          ...
          options: [
            "Deux cultures qui se détestent",
            "Ce sont toutes deux des Solanacées",
            "Le piment refuse la terre noire",
