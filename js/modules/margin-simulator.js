...
export const MarginSimulatorModule = {
  ...
  calculateMargin() {
    ...
    const transportCost = this.storage.calculateTransportCost(
      quantityKg,
      fromRegion,
      toRegion,
      vehicleType
    ...
    const margin = this.storage.calculateNetMargin(
      pricePerKg,
      quantityKg,
      transportCost,
      otherCosts
  ...
  updateComparisonResults() {
    ...
    const otherRegions = this.regions.filter(
      (r) => r !== currentToRegion && r !== this.state.fromRegion
    );

    container.innerHTML =
      '<h4 class="text-sm font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider mb-3">Comparaison avec d\'autres destinations</h4>';

    otherRegions.slice(0, 3).forEach((region) => {
      const transportCost = this.storage.calculateTransportCost(
        quantityKg,
        this.state.fromRegion,
        region,
