...
const DISEASE_LIBRARY = [
  // Tomate diseases
  {
    ...
    description:
      "La mineuse de la tomate (Tuta absoluta) est un papillon dont les chenilles creusent des galeries dans les feuilles et les fruits. Très destructrice, elle peut causer jusqu'à 100% de pertes si non contrôlée.",
    severity: "Critique",
    frequency: "Élevée",
    prevention: [
  ...
  },
  {
    ...
    description:
      "Maladie fongique causée par Phytophthora infestans. Se développe rapidement par temps humide et chaud. Peut détruire une culture en quelques jours.",
    severity: "Critique",
    frequency: "Élevée",
    prevention: [
  ...
  },
  {
    ...
    description:
      "Maladie fongique causée par Erysiphe spp. ou Leveillula taurica. Se développe par temps chaud et sec. Affecte principalement les feuilles.",
    severity: "Moyenne",
    frequency: "Élevée",
    prevention: [
  ...
  },
  {
    ...
    description:
      "Insectes suceurs qui se nourrissent de la sève des plantes. Affaiblissent la plante et peuvent transmettre des virus. Se reproduisent très rapidement.",
    severity: "Élevée",
  ...
  },
  {
    ...
    description:
      "Petits insectes blancs suceurs de sève. Proviennent souvent des serres ou des cultures sous abri. Peuvent transmettre des virus.",
    severity: "Élevée",
    frequency: "Élevée",
    prevention: [
      "Installer des pièges englués jaunes",
  ...
  },
  {
    ...
    description:
      "Maladie fongique du sol causée par Fusarium spp. Très difficile à contrôler une fois établie. Peut survivre dans le sol pendant plusieurs années.",
    severity: "Critique",
    frequency: "Moyenne",
    prevention: [
    ...
    notes:
      "Une fois installée, la fusariose est très difficile à éradiquer. La prévention est la clé.",
  },
  {
    id: "D-008",
    name: "Anthracnose",
  ...
  },
  {
    ...
    notes:
      "Les traitements bactéricides sont peu efficaces une fois la maladie bien installée. La prévention est essentielle.",
  },
  {
    id: "D-010",
    ...
    chemicalTreatments: [],
    notes:
      "Aucun traitement curatif efficace. Les plants infectés doivent être arrachés et brûlés immédiatement.",
  },
];
...
const SYMPTOMS_DATABASE = {
  Tomate: {
    ...
    racines: ["Racines noircies ou pourries", "Plants qui tombent facilement"],
    fleurs: ["Fleurs qui tombent", "Pas de fruits", "Déformation des fleurs"],
    plante_entiere: ["Flétrissement soudain", "Plante qui meurt", "Croissance ralentie"],
  },
  Oignon: {
    feuilles: [
    ...
    tiges: ["Tiges molles", "Tiges noircies"],
    fruits: ["Bulbes pourris", "Bulbes déformés"],
    racines: ["Racines pourries", "Bulbes qui pourrissent"],
    fleurs: [],
    plante_entiere: ["Flétrissement", "Croissance arrêtée"],
  },
  Piment: {
    feuilles: [
      "Taches jaunes ou brunes",
    ...
    tiges: ["Tiges molles", "Vaisseaux bruns"],
    fruits: ["Taches noires sur les fruits", "Fruits déformés", "Trous dans les fruits"],
    racines: [],
    fleurs: ["Fleurs qui tombent"],
    plante_entiere: ["Flétrissement", "Plante qui meurt"],
  },
  Chou: {
    feuilles: [
      "Taches jaunes ou brunes",
    ...
    racines: [],
    fleurs: [],
    plante_entiere: ["Croissance ralentie"],
  },
};
...
const SYMPTOM_TO_DISEASE = {
  Tomate: {
    feuilles: {
      "Galeries ou tunnels": {
        "Mineuse de la Tomate (Tuta Absoluta)": 0.9,
        "Aleurodes (Mouches blanches)": 0.1,
      },
      "Feuilles qui s'enroulent": {
        "Mineuse de la Tomate (Tuta Absoluta)": 0.7,
        Pucerons: 0.6,
        "Virus de la flétrissure bactérienne": 0.3,
      },
    ...
    tiges: {
      "Vaisseaux bruns (en coupant)": {
        Fusariose: 0.9,
        "Virus de la flétrissure bactérienne": 0.8,
      },
...
export const DiagnosticsModule = {
  ...
  renderDiseaseLibrary() {
    ...
    if (cropFilter) {
      filteredDiseases = filteredDiseases.filter((d) => d.cropTypes.includes(cropFilter));
    }
    ...
    tableBody.innerHTML = filteredDiseases
      .map((disease) => {
        const severityInfo = SEVERITY_COLORS[disease.severity] || SEVERITY_COLORS["Moyenne"];
        const frequencyColor =
          disease.frequency === "Très Élevée"
            ? "rose"
            : disease.frequency === "Élevée"
              ? "amber"
              : disease.frequency === "Moyenne"
                ? "blue"
                : "slate";
  ...
  renderHistory() {
    ...
    if (sorted.length === 0) {
      historyList.innerHTML =
        '<p class="text-[11px] text-slate-400 text-center py-4">Aucun diagnostic dans l\'historique</p>';
      return;
    }
    ...
    for (let i = 1; i <= 4; i++) {
      ...
      if (stepCircle) {
        stepCircle.className =
          "step-pending step-circle w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold";
      }
    }
  },
  ...
  showStep(stepNumber) {
    ...
    for (let i = 1; i <= stepNumber; i++) {
      ...
      if (stepCircle) {
        stepCircle.className =
          i < stepNumber
            ? "step-completed step-circle w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-bold"
  ...
  loadSymptoms(crop, affectedPart) {
    ...
    if (symptoms.length === 0) {
      symptomsList.innerHTML =
        '<p class="text-sm text-slate-500">Aucun symptôme enregistré pour cette combinaison.</p>';
      return;
    }
  ...
  performDiagnosis() {
    ...
    const diseaseResults = Object.entries(diseaseScores)
      .map(([disease, score]) => ({
        disease,
    ...
    // Get disease details
    const resultsHTML = diseaseResults
      .map((result) => {
  ...
  saveDiagnostic() {
    ...
    ErrorHandler.showToast(
      `Diagnostic pour "${currentDiagnostic.cropType}" enregistré avec succès ! Vous pouvez le consulter dans l'historique.`,
      "success"
    );
  ...
  quickDiagnose(diseaseName) {
    ...
    currentDiagnostic = {
      ...
      treatmentRecommendations: [...disease.organicTreatments, ...disease.chemicalTreatments],
      notes: "",
      status: "En cours",
      enterprise_id: "ka_farm",
    };
    ...
    if (content) {
      content.innerHTML = `
        <div class="space-y-4">
          ...
          ${
            disease &&
            (disease.organicTreatments.length > 0 || disease.chemicalTreatments.length > 0)
              ? `
            <div class="p-3 bg-amber-500/5 dark:bg-amber-950/5 rounded-xl border border-amber-500/20">
              <p class="text-[10px] font-bold text-amber-500 dark:text-amber-400 uppercase tracking-wider mb-2 flex items-center gap-1">
  ...
  parseMarkdown(text) {
    ...
    text = text.replace(
      /^\d+\.\s*\*\*(.*?)\*\*/gm,
      '<h4 class="text-xs font-black text-emerald-500 uppercase tracking-wider mt-3 mb-1">$1</h4>'
    );
  ...
  async handleAIDiagnosticSubmit() {
    ...
    const userPrompt =
      promptInput.value.trim() || "Identifie le problème sur cette plante et donne des conseils.";
    ...
    try {
      ...
      const sympMatch = text.match(
        /3\.\s*\*\*Symptômes visibles sur l'image\*\*([\s\S]*?)4\.\s*\*\*/i
      );
      if (sympMatch)
        sections.symptomes = sympMatch[1]
          .trim()
          .replace(/^\s*\*\s/gm, "<li>")
          .replace(/$/gm, "</li>");
      ...
      if (recoMatch)
        sections.recommandations = recoMatch[1]
          .trim()
          .replace(/^\s*\*\s/gm, "<li>")
          .replace(/$/gm, "</li>");
