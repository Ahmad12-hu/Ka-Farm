...
export const RotationModule = {
  ...
  startAutoRefresh() {
    ...
    document.addEventListener("ka_data_updated", (e) => {
      if (
        e.detail.key.includes("rotation") ||
        e.detail.key.includes("plant") ||
        e.detail.key.includes("crop")
      ) {
        this.loadData();
        this.render();
      }
  ...
  updateStats() {
    const stats = {
      ...
      "stat-current-crops": crops.filter((c) => c.status !== "Récolté" && c.status !== "Terminé")
        .length,
    ...
    container.innerHTML = parcels
      .map((parcel) => {
        ...
        const progressPercent = lastRotation
          ? (() => {
              const start = new Date(lastRotation.start_date);
              const end = new Date(lastRotation.end_date || lastRotation.start_date);
              const now = new Date();
              const totalDuration = end - start;
              const elapsed = now - start;
              return totalDuration > 0 ? Math.min(100, (elapsed / totalDuration) * 100) : 0;
            })()
          : 0;
  ...
  renderFamilyLibrary() {
    ...
    container.innerHTML = plantFamilies
      .map((family) => {
  ...
  renderCropFamiliesList() {
    ...
    container.innerHTML = cropFamilies
      .map((cf) => {
  ...
  openDeleteRotationModal(recordId) {
    ...
    if (modal) {
      ...
      if (message) {
        message.textContent =
          "Vous êtes sur le point de supprimer cet enregistrement de rotation. Cette action est irréversible.";
      }
    }
  },
  ...
  editFamily(familyId) {
    ...
    document.getElementById("family-compatible").value =
      family.compatible_families?.join(", ") || "";
    document.getElementById("family-incompatible").value =
      family.incompatible_families?.join(", ") || "";
  },
  ...
  savePlantFamily(event) {
    ...
    const compatible =
      document
        .getElementById("family-compatible")
        ?.value.split(",")
        .map((s) => s.trim())
        .filter((s) => s) || [];
    const incompatible =
      document
        .getElementById("family-incompatible")
        ?.value.split(",")
        .map((s) => s.trim())
        .filter((s) => s) || [];
  ...
  showFamilyDetails(familyId) {
    ...
    const compatibleFamilies = family.compatible_families
      .map((id) => {
    ...
    const incompatibleFamilies = family.incompatible_families
      .map((id) => {
        const f = KAStorage.getFamilyById(id);
        return f ? f.name : id;
      })
  ...
  openParcelRotationDetail(parcelId) {
    ...
    if (rotations.length === 0) {
      ErrorHandler.showToast(
        `Aucune rotation enregistrée pour la parcelle : ${parcel.name}`,
        "error"
      );
      return;
    }
