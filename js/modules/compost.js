...
export const CompostModule = {
  ...
  addMaterialToCalculator() {
    ...
    this.state.currentCalculatorMaterials[materialId] =
      (this.state.currentCalculatorMaterials[materialId] || 0) + quantity;
  ...
  renderCalculatorMaterials() {
    ...
    for (const [materialId, quantity] of Object.entries(this.state.currentCalculatorMaterials)) {
      ...
      item.className =
        "p-2.5 bg-slate-100 dark:bg-[#0D2615]/50 rounded-lg border border-slate-200 dark:border-[#143E23]/40 flex items-center justify-between";
      item.innerHTML = `
    ...
    content.innerHTML = `
      <div class="space-y-4">
        ...
        <div class="p-4 bg-slate-50 dark:bg-[#0D2615]/30 rounded-2xl">
          ...
          <div class="space-y-2">
            ${Object.entries(batch.materials_used || {})
  ...
  addMaterialToBatch() {
    ...
    this.state.currentBatchMaterials[materialId] =
      (this.state.currentBatchMaterials[materialId] || 0) + quantity;
  ...
  renderBatchMaterialsSelector() {
    ...
    listContainer.innerHTML = Object.entries(this.state.currentBatchMaterials)
      .map(([materialId, quantity]) => {
        const material = materials.find((m) => m.id === materialId);
        if (!material) return "";
  ...
  saveMaterial(event) {
    ...
    const cnRatio =
      parseFloat(document.getElementById("material-cn-ratio").value) || carbon / nitrogen || 0;
