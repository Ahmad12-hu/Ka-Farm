...
function getEnterpriseId() {
  return localStorage.getItem('ka_user_enterprise_id') || 'ka_farm';
}

function getCollectionName(key) {
  const baseName = KEY_MAP[key];
  if (!baseName) return null;
  if (key === 'ka_farm_users') return 'users';
  const entId = getEnterpriseId();
  return `${entId}_${baseName}`;
}

function getScopedLocalKey(localKey) {
  if (localKey === 'ka_farm_users' || localKey.startsWith('ka_user_')) return localKey;
  const entId = getEnterpriseId();
  return `${entId}_${localKey}`;
}
    ...
    Object.keys(KEY_MAP).forEach((localKey) => {
      ...
      onSnapshot(collRef, async (snapshot) => {
        if (!snapshot.empty) {
          ...
          const scopedLocalKey = getScopedLocalKey(localKey);
          const localDataStr = localStorage.getItem(scopedLocalKey);
          ...
          const scopedLocalKey = getScopedLocalKey(localKey);
          const localData = localStorage.getItem(scopedLocalKey);
          if (localData) {
            try {
