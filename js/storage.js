...
const DEFAULT_PARCELLES = [
  { id: 'P-001', name: 'Parcelle Nord - Planche 2', surface: 120, lat: 14.7932, lng: -17.2654, status: 'Cultivée', history: ['Tomate Mongal F1', 'Chou Cabus', 'Jachère'], currentCrop: 'Tomate Mongal F1' },
  { id: 'P-002', name: 'Parcelle Est - Grand Champ', surface: 500, lat: 14.7938, lng: -17.2642, status: 'Cultivée', history: ['Oignon Rouge de Galmi', 'Piment Oiseau', 'Arachide'], currentCrop: 'Oignon Rouge de Galmi' },
  { id: 'P-003', name: 'Parcelle Sud - Planche 1', surface: 150, lat: 14.7924, lng: -17.2659, status: 'Cultivée', history: ['Chou Cabus', 'Aubergine', 'Jachère'], currentCrop: 'Chou Cabus' },
  { id: 'P-004', name: 'Zone Ombragée - Bac A', surface: 50, lat: 14.7935, lng: -17.2662, status: 'Cultivée', history: ['Menthe de Thiès', 'Laitue de saison'], currentCrop: 'Menthe de Thiès' },
  { id: 'P-005', name: 'Parcelle Ouest - Verger', surface: 800, lat: 14.7928, lng: -17.2648, status: 'En préparation', history: ['Papayer Solo', 'Gombo d\'hivernage'], currentCrop: 'Papayer Solo (Jeunes plants)' }
];
...
export const KAStorage = {
  init() {
    if (!localStorage.getItem('ka_farm_crops')) this.saveCrops(DEFAULT_CROPS);
    if (!localStorage.getItem('ka_farm_nurseries')) this.saveNurseries(DEFAULT_NURSERIES);
    if (!localStorage.getItem('ka_farm_stocks')) this.saveStocks(DEFAULT_STOCKS);
    if (!localStorage.getItem('ka_farm_tasks')) this.saveTasks(DEFAULT_TASKS);
    if (!localStorage.getItem('ka_farm_finances')) this.saveFinances(DEFAULT_FINANCES);
    if (!localStorage.getItem('ka_farm_parcelles')) this.saveParcelles(DEFAULT_PARCELLES);
    if (!localStorage.getItem('ka_farm_users')) this.saveUsers(DEFAULT_USERS);
    if (!localStorage.getItem('ka_farm_employees')) this.saveEmployees(DEFAULT_EMPLOYEES);
    if (!localStorage.getItem('ka_farm_attendance')) this.saveAttendance(DEFAULT_ATTENDANCE);
    if (!localStorage.getItem('ka_farm_employee_payments')) this.saveEmployeePayments(DEFAULT_EMPLOYEE_PAYMENTS);
    if (!localStorage.getItem('ka_farm_cheptel')) this.saveCheptel(DEFAULT_CHEPTEL);
    if (!localStorage.getItem('ka_farm_elevage_production')) this.saveElevageProduction(DEFAULT_ELEVAGE_PRODUCTION);
