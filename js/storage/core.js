...
const DEFAULT_TASKS = [
  ...
  {
    id: "T-403",
    title: "Vérifier la levée de la pépinière tomates",
    category: "Pépinière",
    dueDate: "2026-06-25",
    assignee: "Responsable terrain",
    priority: "Haute",
  ...
  {
    id: "T-404",
    title: "Achat de 5 sacs de fumier de volaille",
    category: "Entretien",
    dueDate: "2026-06-30",
    assignee: "Employé",
    priority: "Basse",
...
const DEFAULT_FINANCES = [
  ...
  },
  {
    ...
    category: "Ventes",
    amount: 120000,
    description: "Vente récolte juin",
    date: "2026-06-28",
  },
];
...
export const KAStorage = {
  ...
  init() {
    ...
    if (migrated) {
      try {
        console.info("[KAStorage] Migration: double-prefixed storage keys normalized.");
      } catch {}
    }
  },
  ...
  getUsers() {
    ...
    const migratedUsers = users.map((user) => {
      if (
        user.password &&
        (user.password.length !== 64 || !/^[0-9a-f]{64}$/i.test(user.password))
      ) {
        hasPlain = true;
